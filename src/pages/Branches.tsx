...
import { createBranch, deleteBranch, listBranches, updateBranch } from '../lib/api/client';
...
export function Branches() {
  ...
  async function handleCreate(payload: BranchPayload) {
    ...
    if (response.error || !response.data) {
      setFormError(response.error?.message ?? 'Unable to create branch.');
      return;
    }

    setFormOpen(false);
    setSavedMessage(`Branch ${response.data.code} created successfully.`);
    await loadBranches();
  }

  async function handleUpdate(payload: BranchPayload) {
    if (!accessToken || !selected) return;
    setFormBusy(true);
    setFormError(null);
    const response = await updateBranch(accessToken, selected.id, payload);
    setFormBusy(false);

    if (response.error || !response.data) {
      setFormError(response.error?.message ?? 'Unable to update branch.');
      return;
    }

    setSelected(response.data);
    setFormOpen(false);
    setSavedMessage(`Branch ${response.data.code} updated successfully.`);
    await loadBranches();
  }
  ...
  async function handleDelete() {
    ...
    setDeleteBusy(true);
    const response = await deleteBranch(accessToken, deleteTarget.id);
    setDeleteBusy(false);
    if (response.error) {
      setError(response.error);
      return;
    ...
    setSavedMessage(`Branch ${deleteTarget.code} deleted successfully.`);
    await loadBranches();
  }

  const columns = useMemo(() => {
    const base: Column<Branch>[] = [
      {
        key: 'code',
        header: 'Code',
        value: (row) => row.code,
  ...
  return (
    <>
      <PageBanner
        title="Branches"
        subtitle="Admin-only branch reference data managed through the centralized auth service"
        breadcrumb={['Kyronix', 'Branches']}
        action={
      ...
      <div className="grid grid-cols-1 gap-5 sm:grid-cols-3">
        <StatCard label="Total branches" value={String(rows.length)} tone="neutral" delta={0} />
      ...
      {!accessToken && (
        <div className="rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">
          No access token is available. Sign in again to manage branches.
        </div>
      )}
      ...
      <DetailDrawer
        open={detailOpen}
        title={selected?.code ?? 'Branch detail'}
        subtitle="Admin branch record"
        onClose={() => setDetailOpen(false)}
        width="md"
      >
        {!selected && <EmptyState description="Select a branch to view its details." />}
        {selected && (
          <dl>
      ...
      <ConfirmModal
        ...
        title={deleteTarget ? `Delete ${deleteTarget.code}?` : 'Delete branch?'}
        description={
          deleteTarget
            ? `This will permanently delete the ${deleteTarget.name} branch record. This action cannot be undone.`
            : undefined
        }
