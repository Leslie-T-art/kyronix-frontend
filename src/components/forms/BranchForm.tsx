...
export function BranchForm({
  ...
  return (
    <FormDrawer
      ...
      formId="branch-form"
      submitLabel={isSubmitting ? 'Saving...' : mode === 'create' ? 'Create branch' : 'Update branch'}
      submitDisabled={isSubmitting || code.trim() === '' || name.trim() === ''}
      onClose={onClose}
      onSubmit={handleSubmit}
    >
      <FormSection title="Branch details">
        <Field label="Code" htmlFor="branch-code" required>
          <TextInput
            id="branch-code"
            value={code}
        ...
        <Field label="Name" htmlFor="branch-name" required>
          <TextInput
            id="branch-name"
            value={name}
